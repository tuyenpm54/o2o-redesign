...
function StorefrontPreviewContent() {
    ...
    const defaultBlocks = [
        ...
        { id: 'menu', type: 'menu-grid', title: 'Thực đơn' }
    ];
    ...
    // Support Chat State Removed - Moved to /chat page
    ...
    const handlePlaceOrder = () => {
        ...
        // In a real app, this would be handled by the /api/orders POST
        // which I've already implemented and called in DiscoveryPage.
        // For single-order-page, we should ideally use the same handlePlaceOrder logic.
    };

    const handlePlaceOrderReal = async () => {
        try {
    ...
    // Chat Logic Migration - See /chat page for implementation
    ...
    const cartTotalPrice = Object.entries(cart).reduce((acc, [id, qty]) => {
        const item = [...MENU_ITEMS, ...COMBOS].find(i => i.id === Number(id));
    ...
    return (
        <div className={styles.container}>
            ...
            {/* Smart Context Banner removed for chat bubble integration */}
            ...
            <div className={`${styles.supportWrapper} ${cartTotalCount > 0 && cartTotalPrice < PROMO_THRESHOLD ? styles.supportShift : ''}`}>
                <div
                    ...
                    style={{ cursor: 'pointer', marginBottom: '8px', zIndex: 100 }}
                >
                    Nhân viên đã nhận yêu cầu và đang tới rồi ạ.
                </div>
                                        ...
                                        displayOrders.map((order, idx) => (
                                            <div key={`all-ord-${idx}`} className={styles.orderCardItem}>
                                                <div className={styles.orderItemInfo}>
                                                    <h4>{order.name || ([...MENU_ITEMS, ...COMBOS].find(i => i.id === Number(Object.keys(order.items || {})[0]))?.name)}</h4>
                            ...
                            <div className={styles.totalRow}>
                                ...
                                <span className={styles.totalPrice}>{(cartTotalPrice + activeOrders.reduce((acc, o) => {
                                    return acc + Object.entries(o.items || {}).reduce((sum: any, [id, qty]: any) => {
                                        const p = [...MENU_ITEMS, ...COMBOS].find(i => i.id == Number(id))?.price || 0;
