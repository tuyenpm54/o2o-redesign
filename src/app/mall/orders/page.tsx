    ...
    return (
        <div className={styles.container}>
            ...
            <header className={styles.header}>
                <Link href="/mall" style={{ textDecoration: 'none' }}>
            ...
            <div className={styles.itemList}>
                {displayedItems.length === 0 ? (
                    ...
                    displayedItems.map((item) => {
                        ...
                        // Mock store name assignment based on ID parity
                        const storeName = item.id.includes('1') ? 'Highlands Coffee' : 'Phở 24';
                                ...
                                <div className={styles.itemContent}>
                                    ...
                                    <div className={styles.metaRow}>
                                        <div className={styles.userMeta}>
                                            <ChefHat size={12} className="inline mr-1 text-slate-400" />
                                            <span className="text-sm font-medium text-slate-600 mr-2">{storeName}</span>
                                            <span className="text-xs text-slate-400">• {item.orderedBy.isCurrentUser ? 'Bạn' : item.orderedBy.name}</span>
                                        </div>
            ...
            <div className={`${styles.billPreviewSection} ${ALL_ITEMS.length === 0 ? styles.disabled : ''}`}>
                ...
                <Link
                    href={ALL_ITEMS.length > 0 ? "/mall/bill" : "#"}
